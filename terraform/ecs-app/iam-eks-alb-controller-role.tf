resource "aws_iam_role" "eks-alb-controller" {
  ...
  name  = "${var.environment}-app-eks-alb-controller"
  ...
  assume_role_policy = <<POLICY
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Condition": {
        "StringEquals": {
            "${aws_iam_openid_connect_provider.main[0].url}:sub": "system:serviceaccount:kube-system:aws-load-balancer-controller",
